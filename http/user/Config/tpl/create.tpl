        ...
        <div class="container-fluid">
            <div class="col-md-4 openm-id-form">
                <form class="form-signin form-group" method="POST" action="{$action}">
                    ...
                    <div class="form-group {if $isResponse}has-feedback {if $error.mail!=null}has-error{else}has-success{/if}{/if}">
                        <label class="control-label" for="mail">{#login_label#}{if $isResponse && $error.mail!=null} {#$error.mail#}{/if}</label>
                        <div class="controls">
                            <input type="email" {if $isResponse && $error.mail!=null}autofocus{/if} required name="mail" class="form-control" id="email" value="{$mail}" placeholder="{#login_text_box#}">{if $isResponse}
                    ...
                    <div class="form-group {if $isResponse && $error.mail==null}has-feedback {if $error.password!=null}has-error{else}{if $error.head!=null}has-warning{else}has-success{/if}{/if}{/if}">
                        <label class="control-label" for="password">{#password_label#}{if $isResponse && $error.password!=null} {#$error.password#}{/if}</label>
                        <div class="controls">
                    ...
                    <div class="form-group {if $isResponse && $error.mail==null}has-feedback {if $error.password2!=null}has-error{else}{if $error.head!=null}has-warning{else}has-success{/if}{/if}{/if}">
                        <label class="control-label" for="password2">{#password_2_label#}{if $isResponse && $error.password2!=null} {#$error.password2#}{/if}</label>
                        <div class="controls">
                    ...
                    <button class="btn btn-success" type="submit"><i class="glyphicon glyphicon-plus"></i>  {#create_label#}</button>
